...
export default function AnimatedText({
  ...
  const startAnimation = () => {
    ...
    sentences.forEach((sentence, sentenceIndex) => {
      ...
      words.forEach((word, wordIndex) => {
        ...
        const wordAppearDelay =
          sentenceBaseDelay + (wordDelay + wordIndex * staggerDelay) * 1000;
  ...
  return (
    <div className={`font-favorit ${className} relative`}>
      ...
      <button
        onClick={handleSkip}
        className="font-gaisyr fixed bottom-8 left-1/2 transform -translate-x-1/2 text-gray-500 border-b border-dotted border-gray-500 hover:text-gray-700 transition-colors"
      >
        skip introductory scene <span className="font-favorit">↗</span>
      </button>
    </div>
  );
}
